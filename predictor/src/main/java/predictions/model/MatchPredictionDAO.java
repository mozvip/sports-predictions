...
public interface MatchPredictionDAO {
	  ...
	  @SqlUpdate("create table if not exists match_prediction ( community varchar(100) not null, email varchar(100) not null, match_id int not null, home_team_id varchar(5) not null, home_score int not null default 0, away_score int not null default 0, away_team_id varchar(5) not null, home_winner boolean, primary key (email, community, match_id))")
	  void createTable();
